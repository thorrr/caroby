...
pushd .
cd "%DOWNLOAD_DIR%
call :download %downloadUrl% || goto :error
set exefile=%CD%\%_rv%
call :verifyMD5Hash "%exefile%" %md5sum% || goto :error
popd
...
pushd .
cd "%_rv%"
7z x -y "%exefile%" >nul || goto :error
:: find top-level subdir with "bin\idea64.exe" folder
for /d /r "%CD%" %%a in (*) do if exist "%%a\bin\idea64.exe" set "binpath=%%a"
move "%binpath%" "%packageDir%"
popd
...
bin\mkshortcut.vbs /target:cmd /args:"/c bin\caroby-init.bat %_rv%\bin\idea64.exe" /shortcut:intellij
popd
