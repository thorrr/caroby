...
set zipfile=%_rv%
call :verifyMD5Hash "%CD%\%zipfile%" %md5sum% || goto :error
popd
...
call :UnZipFile "%_rv%" "%DOWNLOAD_DIR%\%zipfile%" || goto :error
move "%_rv%" "%packageDir%" >nul

:mkshortcut
pushd .
cd /d %CAROBY_DIR%
call :shortInstallPath %packageName% || goto :error
bin\mkshortcut.vbs /target:cmd /args:"/c bin\caroby-init.bat %_rv%\bin\pycharm64.exe" /shortcut:pycharm
popd
