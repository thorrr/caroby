...
pushd .
:::::::::::::::::::::::::::::::::
set URL=http://download.microsoft.com/download/7/9/6/796EF2E4-801B-4FC4-AB28-B59FBF6D907B/VCForPython27.msi
set md5sum=4e6342923a8153a94d44ff7307fcdd1f
:::::::::::::::::::::::::::::::::

set packageName=VCForPython27
...
set fname=%_rv%
call :verifyMD5Hash "%DOWNLOAD_DIR%\%_rv%" %md5sum% || goto :error
popd
...
set installDir=%_rv%
echo Installing...
start /wait msiexec /a "%DOWNLOAD_DIR%\%fname%" TARGETDIR="%installDir%" /qn
::have to rename 9.0 to VC because the python install scripts hardcode
::vcvarsall.bat to be in a folder called "VC"
rename "%installDir%\Microsoft\Visual C++ for Python\9.0" VC
echo Done.

::buildInit
call :initPath %packageName% || goto :error
mkdir "%_rv%"
set initName=%_rv%\vc-init.bat
call :installPathRelative %packageName% || goto :error
set vcDir=%_rv%
>"%initName%" echo @echo off
>>"%initName%" echo.
>>"%initName%" echo set VS90COMNTOOLS=%vcDir%\Microsoft\Visual C++ for Python\VC\VC
